...
.factory('SystemService', function($http, $state) {
  return {
  	logout: function() {
    ...
    // check: function($scope) {
      // $http.post('http://chanmao.ca/index.php?r=MobOrder/historylist', { uid: 15 })
 ...
 return {
    login: function($scope, $ionicPopup) {	
	  ...
	  if (errortext != ''){	  		  
      	...
      	$http.post('http://chanmao.ca/index.php?r=MobLogin/login', { username: $scope.login.username, password: $scope.login.password })
      	.success(function(data, status, headers, config) {
	...
	register: function($scope, $ionicPopup) {
	  ...
	  if (errortext != ''){	  		  
	      ...
	      $http.post('http://chanmao.ca/index.php?r=MobLogin/register', { username: $scope.register.username, password: $scope.register.password, email: $scope.register.email })
	      .success(function(data, status, headers, config) {
	            $scope.result = data.result;  
	            if (data.result == 2)
    ...
    forget: function($scope, $ionicPopup, mode) {	
	  ...
	  if (errortext != ''){	  		  
	      ...
	      $http.post('http://chanmao.ca/index.php?r=MobLogin/forget', { email: $scope.forget.email, mode: mode })
	      .success(function(data, status, headers, config) {
	           	 $scope.result = data.result ; 
	           	 errortext = data.errorcontent;
	           }).error(function(data, status) { 
...
.factory('RRService', function($http, $state, $rootScope) {
 return {
 	gotoRR	: function(rid) {
    ...
    rrlist	: function($scope) {
      ...
      $http.get('http://chanmao.ca/index.php?r=MobOrder/Rrlist')
      .success(function(data, status, headers, config) {
            if (data.result== 1)
           	   $scope.types = [{ 'type':1, 'open': 1,'desc': '营业中','rrs': data.open }, { 'type':0, 'open': 0,'desc': '尚未营业', 'rrs': data.close }];
    ...
    rrmenu: function($scope) {
      ...
      // $http.post('http://chanmao.ca/index.php?r=MobOrder/Rrmenu', { rid: rid })
      // .success(function(data, status, headers, config) {
      //       if (data.result== 1)
      ...
      //      });  
        $http({
          method: 'POST',
          url: 'http://chanmao.ca/index.php?r=MobOrder/Rrmenu',
          data:{ rid: rid }
...
.factory('OrderService', function($http, $location, $state,constant,$rootScope) {
 return {
    ...
    delifee : function($scope) {
      ...
      if ($scope.dltype == 1) {
      	 rid = window.localStorage.getItem("sv_rid");
      	 $http.post('http://chanmao.ca/index.php?r=MobOrder/calcdeli', { rid: parseInt(rid), uaid: parseInt($scope.uaid) })
      		.success(function(data, status, headers, config) {
            	$scope.result = data.result; 
    ...
    checkout : function($scope) {
      ...
      // if ($scope.dltype == 1) {
      	 ...
      	 $http.post('http://chanmao.ca/index.php?r=MobOrder/checkout', { 
      	 						rid: parseInt(rid), 
...
.factory('HistoryService', function($http, $state,$rootScope) {
 return {
    load: function($scope, mode) {  	
      uid = window.localStorage.getItem("sv_uid");  
      $http.post('http://chanmao.ca/index.php?r=MobOrder/historylist', { uid: uid })
      .success(function(data, status, headers, config) {
             $scope.current = data.current;
           	 $scope.orders = data.historylist; 
           	 $scope.available = data.available; 
           	 $scope.rid = data.rid; 
           	 if (mode == 1) $scope.$broadcast('scroll.refreshComplete');
           }).error(function(data, status) { 
              $rootScope.noNetwork(); 
           }).then(function(){
...
.factory('AddressService', function($http, $state,$rootScope) {
 return {
    all: function($scope) {
    	uid = window.localStorage.getItem("sv_uid");  
      $http.post('http://chanmao.ca/index.php?r=MobAddress/addresslist', { uid: uid  })
      .success(function(data, status, headers, config) {
            if (data.result == 1)
    ...
    exist: function($scope) {
    	uid = window.localStorage.getItem("sv_uid"); 
    	exist = 0; 
      $http.post('http://chanmao.ca/index.php?r=MobAddress/addressexist', { uid: uid  })
      .success(function(data, status, headers, config) {
            if (data.result == 1)
           	exist = data.exist; 
           }).error(function(data, status) { 
	...
	create: function($scope) {
      ...
      $http.post('http://chanmao.ca/index.php?r=MobAddress/Addresscreate',  $scope.addr )
      .success(function(data, status, headers, config) {
            if (data.result == 1)
           	 $scope.result = data.result;          	 
           }).error(function(data, status) { 
